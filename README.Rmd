...
observed using the following logistic regression: $logit{\pi_i(\boldsymbol{\phi})} = \phi_0 + \phi_1 z_{i} + \phi_2 M_i +  \phi_3 Y_i + \phi_4 \delta_i$. We estimate the coefficients using two-step GMM estimation based on the unbiased estimating $$ \frac{1}{n} \sum_{i=1}^{n} \eta_i [ \frac{V_i}{\pi_i(\phi)} - 1 ] $$,
